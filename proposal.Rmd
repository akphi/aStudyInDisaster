...
#7. Response Variable:   
Our response variable will be "survival", which indicates whether or not the person survived, which is represented using 0's and 1's, where 0 means that the person did not survive, and 1 means the person did. 
...
Our explanatory variables include passenger class, gender, age, number of siblings and spouses, number of parents and children, the ticket fare and the port of embarkation. We believe that those variables might play a role in the survival chance of the passenger on the Titanic. Other variables such as passenger name, ticket number are all different and might not be so useful for our study. The cabin number is potentially an important variable if we can figure out the layout of the cabin on the ship so as to figure out the distance/ time it take to go to the rescue boat point on the ship.
